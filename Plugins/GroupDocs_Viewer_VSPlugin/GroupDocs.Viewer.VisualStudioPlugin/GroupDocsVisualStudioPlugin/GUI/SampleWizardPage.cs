...
using System.Xml;
using GroupDocsViewerVisualStudioPlugin.XML;
...
namespace GroupDocsViewerVisualStudioPlugin.GUI
{
    public partial class SampleWizardPage : Form
    {
        ...
        public SampleWizardPage(DTE2 application)
        {
            ...
            PopulateTreeView(repoPath + "/Examples/" + (rdbCSharp.Checked ? "GroupDocs.Viewer.Examples.CSharp" : "GroupDocs.Viewer.Examples.VisualBasic"));
        ...
        bool CopyAndCreateProject()
        {
            ...
            //string sampleSourcePath = "GroupDocs.Viewer";
            ...
            try
            {
                ...
                {
                    //For CSharp Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.CSharp\\GroupDocs.Viewer.Examples", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.CSharp\\GroupDocs.Viewer.Examples", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.CSharp\\GroupDocs.Viewer.Examples", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);

                    //For VBasic Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.VisualBasic", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.VisualBasic", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "GroupDocs.Viewer.Examples.VisualBasic", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);
                }

                progressBar1.Value = 50;

                string[] projectFiles = Directory.GetFiles(Path.Combine(destinationPath, (rdbCSharp.Checked ? "GroupDocs.Viewer.Examples.CSharp\\GroupDocs.Viewer.Examples" : "GroupDocs.Viewer.Examples.VisualBasic")), (rdbCSharp.Checked ? "*.csproj" : "*.vbproj"));
